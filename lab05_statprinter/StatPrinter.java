...
public class StatPrinter
{
  ...
  public StatPrinter( ArrayList <Integer> data )       //Time Complexity: O(n) 
  { 
    _frequency = new ArrayList<Integer>();

    int numElements = max(data) + 1;
	...
	_frequency.set( element, _frequency.get(element) + 1 );
    }
  }
  ...
  public Integer max( ArrayList <Integer> data )     //Time Complexity: O(n)
  { 
    int maxInt = 0;
    for (Integer element : data){
	if (element > maxInt)
  ...
  //    isLocalMode(1) -> true
  //    isLocalMode(5) -> true
  public boolean isLocalMode( int i )     //Time Complexity: O(1)
  { 
    if (i > 0 &&
	i < _frequency.size() - 1 &&
  ...
  public ArrayList<Integer> getLocalModes()    //Time Complexity: O(n)
  {
  ...
  public void printHistogram( int longestBar )   //Time Complexity: O(n^2)
  {
    for (int i = 0; i < _frequency.size(); i++) {
	System.out.print( i + " : ");
	double prop = (double)longestBar/max(_frequency);
        int j = 0;
	while ( j < prop * _frequency.get(i) ) {
		System.out.print("*");
		j++; 
	}
	System.out.println();
